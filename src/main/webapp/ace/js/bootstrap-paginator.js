...
(function($) {
		...
		return this.each(function() {
			...
			$(this).empty().append(
					renderpager(parseInt(options.current_page),
							parseInt(options.page_count),
							parseInt(options.button_number),
							options.pager_click).children());
			// append quick jump
			$(this).append(
					quickjump(parseInt(options.current_page),
							parseInt(options.page_count),
							parseInt(options.button_number),
							options.pager_click));
	...
	function renderpager(current_page, page_count, button_number, pager_click) {
		...
		// add first & prev
		$pager.append(
				renderButton('first', current_page, page_count, pager_click))
				.append(
						renderButton('prev', current_page, page_count,
								pager_click));
		// 获取页码范围
		...
		for (var page = page_min; page <= page_max; page++) {
			$pager.append(renderButton(page, current_page, page_count,
					pager_click));
		}
		// add last & next
		$pager.append(
				renderButton('next', current_page, page_count, pager_click))
				.append(
						renderButton('last', current_page, page_count,
								pager_click));
		return $pager;
	}
	// add quick jump
	function quickjump(current_page, page_count, button_number, pager_click) {
		...
		for (var i = 1; i <= page_count; i++) {
			if (i == current_page)
				...
				var attr = "";
			select.append('<option ' + attr + ' value="' + i + '">' + i
					+ '</option>');
		}
		...
		return jump
				.append(
						'<span style="margin-left:15px;vertical-align: middle;">跳转至 </span')
				.append(select);
	}
	...
	function renderButton(link_page, current_page, page_count, pager_click) {
		...
		return btn = $(
				'<li class="' + class_li + '"><a data-toggle="tooltip" '
						+ title + ' href="#">' + link_content + '</a></li>')
				.click(callback);
	}
	// pager defaults.
