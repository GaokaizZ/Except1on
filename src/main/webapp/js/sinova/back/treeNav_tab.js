// JavaScript Document
	...
	$(function(){
		$("#nav_2 span").click(function(){
			...
			temp.show().siblings().hide();
			$(this).parent().hide();
			$("#disNav").attr("src","images/navSelect_1.gif");
		});
	...
	$(function(){
		...
		$("#disNav").live("click",function(){
			...
			//$("#nav_2").css({"top":35})	
			if($(this).attr("src")=='images/navSelect_1.gif'){
				$(this).attr("src","images/navSelect_2.gif");
				$("#nav_2").show();
			}else{
				$(this).attr("src","images/navSelect_1.gif");
				$("#nav_2").hide();	
			}							   
		})
		//点击显示隐藏一级导航
		...
		$(".leftTree").find("span").live("click",function(){
			if($(this).parent().find("ul:first").hasClass("undis")){
				...
				$(this).css("background","url(images/up_bg.gif) no-repeat left center");
			}else{
				$(this).parent().find("ul:first").addClass("undis");
				$(this).css("background","url(images/down_bg.gif) no-repeat left center");
			};
		...
		$(".leftTree").find("span").live("click",function(){
			...
			if(aHref=='null' || aHref.substring(aHref.length-4,aHref.length)  == "null"){//判断导航name值是否为空
				//。。。
			}else{
				//点击左侧导航 动态增加iframe
				//$("#iframeRight").children().addClass("undis");
				$("#iframeRight").html("<iframe frameborder='0'  width='100%' height='100%' scrolling='yes' style='overflow-x: hidden' src=" + aHref +" class=" + aHref + "></iframe>");
		...
		if(url.length>2){//判断导航name值是否为空
			if(tabliTEXT.toLowerCase().indexOf(name.toLowerCase())==-1){//判断主菜单是否有相同的菜单
				if(i<11){//判断主菜单是否超过10个
					$(".pcs_tab_ul").find("li:last").after("<li class='cur'><label title='"+ name +"' name=" + url +"><span><em class='myspan'>"+name+"</em><img src='images/yqClose.gif'/></span></label></li>");
					//清除当前选中状态
