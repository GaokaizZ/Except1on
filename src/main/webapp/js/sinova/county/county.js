...
function commonSaveForma(){
	...
	var countyName = jQuery("#countyName_").val();
	var state = jQuery("#state_").val();
	if(countyName==""||countyName==null){
		alert("请把资料填写完整！");
		jQuery("#countyName_").focus();
		return;
	}
	if(!checkCE(countyName)){
		alert("区县名称请使用汉字或字母！");
		jQuery("#countyName_").focus();
		return;
	}
	$.ajax({
		url:ctx + "/county/save",
		type:"post",
		data:"regionCode="+regionCode+"&countyCode="+countyCode+"&countyName="+countyName+"&state="+state,
		dataType:"text",
		success:function(msg){
			if(msg=="0"){
				alert("恭喜你！添加成功。");
				window.location.href = ctx + "/county/list";
			}
			if(msg=="1"){
				alert("请把资料填写完整!");
			}
...
function commonUpdateForma() {
	...
	if(confirm("确定修改数据吗?")){
		...
		$.ajax({
			url:ctx + "/county/update",
			type:"post",
			data:"state="+state+"&id="+id+"&countyName="+countyName,
			dataType:"text",
			success:function(msg){
				if(msg=="0"){
					alert("恭喜你！修改成功。");
...
function deleteMore(){
	...
	var url = ctx + "/county/delete/more";
...
function cleanqrcodes(){
	...
	jQuery("#countyName_").val("");
	jQuery("#state_").val("");
}

//编辑
function beforeUpdate(id){
	var url = ctx + "/county/beforeUpdate?id="+id;
