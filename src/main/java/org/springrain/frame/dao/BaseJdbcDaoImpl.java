...
import org.springrain.frame.util.StringHelper;
...
public abstract class BaseJdbcDaoImpl extends BaseLogger implements IBaseJdbcDao {
	private static String FRAME_JDBC_CALL_KEY = "frame_jdbc_call_key";
	...
	public <T> List<T> queryForListByProc(Finder finder, Class<T> clazz) throws Exception {
		...
		logger.debug("queryForListByProc：procName={}, functionName={}", procName, functionName);

		if (StringUtils.isBlank(procName) && StringUtils.isBlank(functionName)) {
			logger.error("存储过程和函数不能同时为空!");
		...
		Map<String, ?> params = finder.getParams();
		if (params != null) {
			if (ClassUtils.isBaseType(clazz)) {
				map = simpleJdbcCall.returningResultSet(FRAME_JDBC_CALL_KEY, new RowNumberSingleColumnRowMapper<T>(clazz)).execute(params);
			} else {
				map = simpleJdbcCall.returningResultSet(FRAME_JDBC_CALL_KEY, BeanPropertyRowMapper.newInstance(clazz)).execute(params);
			}
		} else {
			if (ClassUtils.isBaseType(clazz)) {
				map = simpleJdbcCall.returningResultSet(FRAME_JDBC_CALL_KEY, new RowNumberSingleColumnRowMapper<T>(clazz)).execute();
			} else {
				map = simpleJdbcCall.returningResultSet(FRAME_JDBC_CALL_KEY, BeanPropertyRowMapper.newInstance(clazz)).execute();
			}
		}
		return (List<T>) map.get(FRAME_JDBC_CALL_KEY);
	}
	...
	public List<Map<String, Object>> queryForList(Finder finder, Page page) throws Exception {
		...
		if (pageSql == null) {
			logger.warn("pagesql is null, return null.");
			return null;
		}
	...
	public <T> List<T> queryForList(Finder finder, Class<T> clazz, Page page) throws Exception {
		...
		if (ClassUtils.isBaseType(clazz)) {
			if (getDialect().isRowNumber()) {
				return getReadJdbc().query(pageSql, finder.getParams(), new RowNumberSingleColumnRowMapper<T>(clazz));
			} else {
	...
	public <T> List<T> findListDataByFinder(Finder finder, Page page, Class<T> clazz, Object queryBean) throws Exception {
		...
		if (finder == null) {
			...
			logger.debug("finder is null, tableName={}", tableName);
		...
		if (datas == null) {
			logger.warn("datas is null, return [new ArrayList<T>()]");
	...
	public Finder getFinderOrderBy(Finder finder, Page page) throws Exception {
		if (finder == null || page == null) {
			logger.warn("finder or page is null, return finder");
			return finder;
		}

		String order = page.getOrder();
		if (StringUtils.isBlank(order)) {
			logger.warn("page.order is null, return finder");
			return finder;
		}
		...
		if (order.indexOf(" ") > -1 || order.indexOf(";") > -1) {// 认为是异常的,主要是防止注入
			logger.warn("page.order is illegitmacy, return null");
			return null;
		}
	...
	public Finder getFinderWhereByQueryBean(Finder finder, Object o) throws Exception {
		...
		if (CollectionUtils.isEmpty(whereSQLInfo)) {
			logger.warn("whereSQLInfo list is empty, return finder");
			return finder;
		}

		Object aliasObj = ClassUtils.getPropertieValue(GlobalStatic.frameTableAlias, o);
		String alias = null;
		if (aliasObj != null) {
	...
	private String getPageSql(Page page, Finder finder) throws Exception {
		...
		logger.debug("sql={}, orderSql={}", sql, orderSql);
		
		Map<String, Object> paramMap = finder.getParams();

		// 查询sql、统计sql不能为空
		if (StringUtils.isBlank(sql)) {
			logger.error("sql is blank, return null");
			return null;
		}
		...
		if (page == null) {
			logger.warn("page is null, return sql");
		...
		if (finder.getCountFinder() == null) {
			...
			int orderint = RegexValidateUtils.getOrderByIndex(countSql);
			if (orderint > 1) {
				countSql = countSql.substring(0, orderint);
			}
	...
	private String warpsavesql(Object entity, Map<String, Object> paramMap, Boolean isSequence) throws Exception {
			...
			if (fdName.equals(pkName)) {// 如果是ID,自动生成UUID
				...
				if (_getId == null) {
					if (returnType == String.class) {
						...
						String _sequenceValue = entityInfo.getPksequence();
						
						if ((i + 1) == fdNames.size()) {
							// 2015-01-05 modified
							sql.append(StringHelper.underscoreName(fdName)).append(")");
							//sql.append(fdName).append(")");
							
							valueSql.append(_sequenceValue).append(")");
							break;
						}
	...
	public Object save(Object entity) throws Exception {
		...
		if (auditLog == null) {
			logger.debug("auditLog is null, return id={}", id);
			return id;
		}
		...
		auditLog.setCurValue(StringUtils.replace(entity.toString(), NULL, EMPTY));
	...
	@Override
	public List<Integer> update(List<Object> list) throws Exception {
		return update(list, false);
	}
	
	
	@Override
	public List<Integer> update(List<Object> list, boolean onlyupdatenotnull) throws Exception {
		if (CollectionUtils.isEmpty(list)) {
			logger.warn("update list is empty, return null");
			return null;
		}
		
		Map<String, ?>[] maps = new HashMap[list.size()];
		String sql = null;
	...
	@Override
	public List<Integer> save(List<Object> list) throws Exception {
		if (CollectionUtils.isEmpty(list)) {
			logger.warn("save list is empty, return null");
			return null;
		}
	...
	private String warpupdatesql(Object entity, Map<String, Object> paramMap,boolean onlyupdatenotnull) throws Exception {
		...
		// 2015-01-05 modified
		StringBuffer whereSQL = new StringBuffer(" WHERE ").append(StringHelper.underscoreName(pkName)).append("=:").append(pkName);
		...
		for (int i = 0; i < fdNames.size(); i++) {
            ...
            //添加需要更新的字段
			// 2015-01-05 modified
			sql.append(StringHelper.underscoreName(fdName)).append("=:").append(fdName).append(",");
	...
	public Integer update(Object entity, boolean onlyupdatenotnull) throws Exception {
		...
		IAuditLog auditLog = getAuditLog();
		if (auditLog != null) {
			oldEntity = findByID(id, clazz, tableExt);
		}
		// 更新entity
		int hang = getWriteJdbc().update(sql.toString(), paramMap);
		if (auditLog == null) {
			return hang;
		}
		...
		auditLog.setOperationClassId(id.toString());
		auditLog.setOperationTime(new Date());
		auditLog.setPreValue(StringUtils.replace(oldEntity.toString(), NULL, EMPTY));
		auditLog.setCurValue(StringUtils.replace(entity.toString(), NULL, EMPTY));

		String auditTableExt = tableExt;
		if (StringUtils.isBlank(tableExt)) {
			int year = Calendar.getInstance().get(Calendar.YEAR);
			auditTableExt = GlobalStatic.tableExt + year;
		}
		auditLog.setExt(auditTableExt);
		// 保存日志
		saveNoLog(auditLog);
		return hang;
	...
	public <T> T findByID(Object id, Class<T> clazz, String tableExt) throws Exception {
		...
		sql.append(tableName).append(" WHERE ").append(idName).append("=:id");
	...
	public void deleteById(Object id, Class<?> clazz) throws Exception {
		if (id == null) {
			logger.warn("deleteById: id is null, return");
			return;
		}
		EntityInfo entityInfo = ClassUtils.getEntityInfoByClass(clazz);
		String tableName = entityInfo.getTableName();
		...
		if (auditLog == null) {
			logger.debug("auditLog is null, return");
			return;
		}
		if (entityInfo.isNotLog()) {
			logger.debug("entityInfo is NotLog, return");
			return;
		}
		...
		auditLog.setOperationClassId(id.toString());
		auditLog.setOperationTime(new Date());
		auditLog.setPreValue(StringUtils.replace(findEntityByID.toString(), NULL, EMPTY));
	...
	@Override
	public void deleteByIds(List<String> ids, Class<?> clazz) throws Exception {
		if (CollectionUtils.isEmpty(ids)) {
			logger.warn("deleteByIds: ids list is empty, return");
			return;
		}
		EntityInfo entityInfo = ClassUtils.getEntityInfoByClass(clazz);
		String tableName = entityInfo.getTableName();
		...
		sql.append(tableName).append(" WHERE ").append(idName).append(" in (:ids)");
		logger.debug("sql={}", sql);
		Finder finder = new Finder(sql);
	...
	public Map<String, Object> queryObjectByProc(Finder finder) throws Exception {
		String procName = finder.getProcName();
		String functionName = finder.getFunName();
		logger.debug("queryObjectByProc: procName={}, functionName={}", procName, functionName);
	...
	public <T> T queryForObjectByProc(Finder finder, Class<T> clazz) throws Exception {
		...
		if (StringUtils.isEmpty(procName)) {
			logger.warn("queryForObjectByProc: procName is blank, return null");
			return null;
		}
	...
	public <T> T queryForObjectByByFunction(Finder finder, Class<T> clazz) throws Exception {
		...
		if (StringUtils.isEmpty(funName)) {
			logger.warn("queryForObjectByByFunction: funName is blank, return null");
			return null;
		}
