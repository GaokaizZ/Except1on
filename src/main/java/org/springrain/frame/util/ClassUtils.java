...
public class ClassUtils {
	...
	public static List<WhereSQLInfo> getWhereSQLInfo(Class<?> clazz) throws Exception {
		...
		for (String name : names) {
			...
			// 2014-12-30 16:54 modify
			if (StringUtils.isNotBlank(ws.column())) {
				info.setWheresql(StringUtils.replacePattern(ws.sql(), ".*=:", ws.column() + "=:"));
			} else {
	...
	public static List<String> getAllDBFields(Class<?> clazz) throws Exception {
		...
		for (String fdName : allNames) {
			...
			if (isDB == false) {
				...
				// 把驼峰改成大写字母加下划线分隔的样式， 2015-01-05 modified
				//dbList.add(StringHelper.underscoreName(fdName));
			}
		}
	 ...
	 * @throws Exception
	 */
	public static boolean isAnnotation(Class<?> clazz, String fdName, Class<? extends Annotation> annotationClass) throws Exception {
	...
	public static String getTableExt(Object o) throws Exception {
		...
		TableGroup group = clazz.getAnnotation(TableGroup.class);
		String p = group.name();
