...
public class DicDataServiceImpl extends BaseSpringrainServiceImpl implements IDicDataService {
	...
	public void deleteDicDataById(String id, String pathtypekey) throws Exception {
		...
		// 删除其下级
		Finder childFinder = Finder.getDeleteFinder(DicData.class).append(" where pid=:id ");
		childFinder.setParam("id", id);
		super.update(childFinder);
	}
	...
	@Override
	public String findDicByCode(String code) throws Exception {
		Finder finder = Finder.getSelectFinder(DicData.class).append(" WHERE 1=1 ");
		if(StringUtils.isNotBlank(code)){
			finder.append("and code=:code");
			finder.setParam("code", code);
		}
		List<DicData>  ds = super.queryForList(finder, DicData.class);
		if(ds != null && ds.size() == 1){
			return ds.get(0).getRemark();
		}else {
			return null;
		}
