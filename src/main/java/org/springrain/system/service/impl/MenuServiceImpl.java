...
public class MenuServiceImpl extends BaseSpringrainServiceImpl implements IMenuService {
	 ...
	 */
	@Override
	public void deleteMenuAndChildById(String id) throws Exception {

		if (StringUtils.isBlank(id)) {
			logger.warn("primary key[{}] is null, return", id);
			return;
		}

		// 删除自身
		Finder finder = Finder.getDeleteFinder(Menu.class).append(" where id=:id ");
		finder.setParam("id", id);
		super.update(finder);

		// 删除其下级
		Finder childFinder = Finder.getDeleteFinder(Menu.class).append(" where pid=:id ");
