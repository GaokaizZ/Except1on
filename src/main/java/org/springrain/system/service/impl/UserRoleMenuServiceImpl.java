...
public class UserRoleMenuServiceImpl extends BaseSpringrainServiceImpl implements IUserRoleMenuService {
	...
	public List<Role> findRoleByUserId(String userId) throws Exception {
		...
		Finder finder = new Finder("SELECT r.* from ").append(Finder.getTableName(Role.class)).append(" r,").append(Finder.getTableName(UserRole.class)).append("  re where re.user_id=:userId and re.role_id=r.id order by r.id");
	...
	public List<Menu> findMenuByRoleId(String roleId) throws Exception {
		...
		Finder finder = new Finder("SELECT m.* from ").append(Finder.getTableName(Menu.class)).append(" m,").append(Finder.getTableName(RoleMenu.class)).append("  re where re.role_id=:roleId and re.menu_id=m.id order by m.id ");
	...
	public List<User> findUserByRoleId(String roleId) throws Exception {
		...
		Finder finder = new Finder("SELECT u.* from ").append(Finder.getTableName(User.class)).append(" u,").append(Finder.getTableName(UserRole.class)).append("  re where re.role_id=:roleId and re.user_id=u.id");
		finder.setParam("roleId", roleId);
	...
	private Finder getMenuFinderByUserId(String userId, Integer menutype) throws Exception {
		...
		Finder finder = new Finder("SELECT m.* from ").append(Finder.getTableName(Menu.class)).append(" m,").append(Finder.getTableName(RoleMenu.class)).append("  rm,").append(Finder.getTableName(UserRole.class)).append("  ur where ur.user_id=:userId and ur.role_id=rm.role_id and m.id=rm.menu_id and m.state=:state ");
		if (menutype != null) {
			finder.append(" and m.type=:menutype ").setParam("menutype", menutype);
		}
		finder.append(" order by m.sort asc ");
		finder.setParam("userId", userId).setParam("state", "1");
	...
	@Override
	// @Caching(evict={@CacheEvict(value = GlobalStatic.qxCacheKey,key = "'findMenuByRoleId_'+#roleId"),@CacheEvict(value = GlobalStatic.qxCacheKey,key =
	// "'findRoleAndMenu_'+#roleId"),@CacheEvict(value = GlobalStatic.qxCacheKey,key = "'findAllRoleAndMenu'")})
	...
	public void updateRoleMenu(String roleId, String[] menus) throws Exception {
		...
		Finder finder = Finder.getDeleteFinder(RoleMenu.class).append("  where role_id=:roleId ");
		finder.setParam("roleId", roleId);
		this.update(finder);
